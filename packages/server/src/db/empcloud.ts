...
// DB's `api_keys` table. emp-exit reads that SAME table to validate a key — no
...
//
// emp-exit gates by role (authorize()), so we only need the owner's role here;
// no permission resolution is required. Mirrors EmpCloud's
 ...
 * yet (EmpCloud migration 078 not applied) so emp-exit never 500s on auth.
 */
